<template>
        ...
        <form class="p-6 space-y-6" @submit.prevent="submitWithdrawalData">
          ...
          <div class="bg-gray-800 p-6 rounded-lg shadow-md">
            ...
            <div v-if="filePreview" class="flex flex-col items-center justify-center text-center">
              ...
              <button type="button" class="mt-4 bg-yellow-500 text-white py-2 px-4 rounded-lg hover:bg-yellow-400" @click="showModal = true">Change Receipt</button>
            </div>
            <div v-else class="border-2 border-dashed border-gray-600 p-10 flex flex-col items-center justify-center rounded-lg text-center">
              ...
              <button type="button" class="mt-4 text-white bg-blue-500 hover:bg-blue-400 text-sm py-2 px-4 rounded-md" @click="triggerFileUpload">Upload File</button>
            </div>
          </div>
  ...
  const submitWithdrawalData = () => {
    const payload = { amount: form.value.amount, wallet: computedWalletAddress.value, proof: base64FileData.value, type: 'Deposit' }
    setFormData(payload)
  ...
  const computed_wallet_info = computed(() => {
    if (adminData?.value && Object.keys(adminData.value).length) {
      return [
        {
          name: 'Bitcoin',
