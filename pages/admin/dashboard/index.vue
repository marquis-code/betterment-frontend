    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      <div class="col-span-1 md:col-span-2 lg:col-span-2 bg-gray-800 rounded-lg p-6 shadow-lg">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-white text-3xl font-bold mb-4">{{ formatToDollar(userData?.accountBalance) ?? '0.00'}}</p>
            <button @click="router.push('/dashboard/wallet')" class="bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600 transition duration-300">
              Visit Wallet →
            </button>
          </div>
      ...
      <div class="bg-gray-800 rounded-lg p-6 shadow-lg flex justify-between">
       <div>
        ...
        <p class="text-white text-2xl font-bold">{{usersList?.length}}</p>
       </div>
        <img src="@/assets/img/dash2.png" class="h-10 w-10 rounded-full" />
      </div>
      <div class="bg-gray-800 rounded-lg p-6 shadow-lg flex justify-between">
        <div>
          <h3 class="text-white text-lg font-semibold mb-2">Total Transactions</h3>
        <p class="text-white text-2xl font-bold">{{adminTransactionsList?.length}}</p>
        </div>
      ...
      <div class="bg-gray-800 rounded-lg p-6 shadow-lg flex justify-between">
       <div>
        ...
        <p class="text-white text-2xl font-bold">{{formatToDollar(userData?.totalWithdrawals) ?? '0.00'}}</p>
       </div>
      ...
      <div class="bg-gray-800 rounded-lg p-6 shadow-lg flex justify-between">
       <div>
        ...
        <p class="text-white text-2xl font-bold">{{formatToDollar(userData?.totalDeposits) ?? '0.00'}}</p>
       </div>
...
const { 
  loading: loadingTransactionHistory,
    transactionsList: adminTransactionsList,
    depositTransactions,
...
const {  usersList, loading: loadingUsers} = useUsers()
